Best2 앱 개발 계획 (PLAN.md) - v4.4 (2025-11-22)
...
'과거의 오늘' 정리 (신규, v4.2): 홈 화면에 "N년 전 오늘의 추억" 카드를 노출하고, 클릭 시 해당 날짜 사진만 분석하는 기능 추가.
...
('스크린샷 클리너'는 Milestone 8로 이동하여 완료 처리)

5. 수익화 및 커뮤니티 (Monetization & Growth)

AdMob 광고 정책 (베타부터 적용):

띠 배너: ReviewScreen 등 주요 작업 화면 하단에 상시 노출.

전면 광고: '가치 기반 노출'. 누적 분석 사진 50~100장 달성 시 1회 노출 (DB에 카운트 누적).

자사 커뮤니티 홍보 (Rolling Banner) (신규):

위치: '설정' 화면 최하단.

형태: 자동 롤링(Auto-scrolling) 배너. 여러 개의 배너가 일정 시간 간격으로 자동 전환됨.

구현: ViewPager2 + Timer 활용.

관리: Firebase Remote Config를 사용하여, 앱 업데이트 없이 배너 이미지와 링크를 실시간으로 변경/추가 가능하도록 구현.

내용: 질환별 카피, 사용 팁, 커뮤니티 홍보 등 다양한 메시지를 로테이션 노출.
...
위 앱들이 없으면, 프롬프트를 클립보드에 자동 복사하고 "내용이 복사되었습니다" 토스트를 띄움.

이후 웹 브라우저로 gemini.google.com을 엶. (웹사이트는 URL을 통한 자동 입력이 불가능하므로 복사 방식으로 대체)
